...
export function NewIssueDialog() {
  ...
  useEffect(() => {
    ...
    if (newIssueDefaults.title) {
      ...
      setAssigneeValue(resolveDraftAssigneeValue(newIssueDefaults, draft));
          ...
          <textarea
            ...
            onKeyDown={(e) => {
              ...
              if (e.key === "Tab" && !e.shiftKey) {
                e.preventDefault();
                const nextTarget = nextTitleTabTarget({ assigneeValue, projectId });
                if (nextTarget === "description") {
                  descriptionEditorRef.current?.focus();
                } else if (nextTarget === "project") {
          ...
          <div className="overflow-x-auto overscroll-x-contain">
            <div className="inline-flex items-center gap-2 text-sm text-muted-foreground flex-wrap sm:flex-nowrap sm:min-w-max">
              ...
              <InlineEntitySelector
                ...
                onConfirm={() => {
                  const nextTarget = nextAssigneeConfirmTarget(projectId);
                  if (nextTarget === "description") {
                    descriptionEditorRef.current?.focus();
                  } else {
                    projectSelectorRef.current?.focus();
                ...
                renderOption={(option) => {
                  ...
                  const assigneeSelection = parseAssigneeValue(option.id);
                  const assignee = assigneeSelection.assigneeAgentId
                    ? (agents ?? []).find((agent) => agent.id === assigneeSelection.assigneeAgentId)
                    : null;
                  return (
                    <>
